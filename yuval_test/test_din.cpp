...
int get_process_log_test() {
    ...
    assert(enable_policy(pid, 3, 234123) == 0);
    assert(set_process_capabilities(pid, 0, 234123) == 0);
...
// simple block
	for (i = 0; i<30; i++){
    assert(set_process_capabilities(pid, 1, 234123) == 0);
    assert(fork() == -1);
    assert(set_process_capabilities(pid, 0, 234123) == 0);
...
// simple one by one
    ...
    assert(set_process_capabilities(pid, 1, 234123) == 0);
    assert(fork() == -1);
    assert(get_process_log(pid, 1, buff) == 0);
    assert(buff[0].syscall_req_level == 2);
    assert(buff[0].proc_level == 1);
int a = buff[0].time;
...
assert(fork() == -1);
	assert(errno == EINVAL);
    assert(get_process_log(pid, 2, buff) == 0);
for (i = 0; i<30; i++){
    assert(set_process_capabilities(pid, 1, 234123) == 0);
    assert(fork() == -1);
    assert(set_process_capabilities(pid, 0, 234123) == 0);
